...
public class PizzaQuestApp {
    ...
    private Type locationListType = new TypeToken<ArrayList<Location>>() {
    }.getType();
    private Type itemListType = new TypeToken<List<Item>>() {
    }.getType();
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(npcJSON, "UTF-8"))) {
            ...
            for (Map.Entry<String, ArrayList<String>> entry : map.entrySet()) {
                ...
                if (JSONnpc.size() == 5) {
                    ...
                    NonPlayerCharacter npc = new NonPlayerCharacter(entry.getKey(), JSONnpc.get(0), JSONnpc.get(1), JSONnpc.get(2), JSONnpc.get(3), JSONnpc.get(4), JSONnpc.get(5));
                    npcList.add(npc);
                }
